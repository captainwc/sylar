...
namespace ns {
    ...
    bool NameServerModule::handleRockRequest(sylar::RockRequest::ptr request, sylar::RockResponse::ptr response,
                                             sylar::RockStream::ptr stream) {
        sylar::Atomic::addFetch(s_request_count, 1);
        switch (request->getCmd()) {
            case (int)NSCommand::REGISTER: return handleRegister(request, response, stream);
            case (int)NSCommand::QUERY: return handleQuery(request, response, stream);
            case (int)NSCommand::TICK: return handleTick(request, response, stream);
            default: SYLAR_LOG_WARN(g_logger) << "invalid cmd=0x" << std::hex << request->getCmd(); break;
        }
        return true;
    }

    bool NameServerModule::handleRockNotify(sylar::RockNotify::ptr notify, sylar::RockStream::ptr stream) {
        return true;
    }
    ...
    bool NameServerModule::handleRegister(sylar::RockRequest::ptr request, sylar::RockResponse::ptr response,
                                          sylar::RockStream::ptr stream) {
        auto rr = request->getAsPB<RegisterRequest>();
        if (!rr) {
            SYLAR_LOG_ERROR(g_logger) << "invalid register request from: " << stream->getRemoteAddressString();
            return false;
        }
...
#define XX(info, attr)                                                                                     \
        ...
        return false;                                                                                      \
            ...
            if (old_value) {
                if (old_value->m_node->getId() != ns_node->getId()) {
                    SYLAR_LOG_ERROR(g_logger)
                        ...
                        << " old.ip=" << old_value->m_node->getIp() << " old.port=" << old_value->m_node->getPort()
            ...
            if (new_value) {
                if (new_value->m_node->getId() != ns_node->getId()) {
                    SYLAR_LOG_ERROR(g_logger)
                        << "invalid register request from: " << stream->getRemoteAddressString()
                        << " new.ip=" << new_value->m_node->getIp() << " new.port=" << new_value->m_node->getPort()
                        << " cur.ip=" << ns_node->getIp() << " cur.port=" << ns_node->getPort();
                    return false;
                }
            } else {
    ...
    void diff(const std::map<std::string, std::set<uint32_t>>& old_value,
              ...
              std::map<std::string, std::set<uint32_t>>& dels, std::map<std::string, std::set<uint32_t>>& news,
    ...
    bool NameServerModule::handleQuery(sylar::RockRequest::ptr request, sylar::RockResponse::ptr response,
                                       sylar::RockStream::ptr stream) {
        auto qreq = request->getAsPB<QueryRequest>();
        if (!qreq) {
            SYLAR_LOG_ERROR(g_logger) << "invalid query request from: " << stream->getRemoteAddressString();
            return false;
        }
    ...
    bool NameServerModule::handleTick(sylar::RockRequest::ptr request, sylar::RockResponse::ptr response,
                                      sylar::RockStream::ptr stream) {
        return true;
    }
