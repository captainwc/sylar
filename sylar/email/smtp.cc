...
SmtpClient::ptr SmtpClient::Create(const std::string& host, uint32_t port, bool ssl) {
    ...
    if (!addr) {
        SYLAR_LOG_ERROR(g_logger) << "invalid smtp server: " << host << ":" << port << " ssl=" << ssl;
        return nullptr;
    }
    addr->setPort(port);
    Socket::ptr sock;
    ...
    if (!sock->connect(addr)) {
        SYLAR_LOG_ERROR(g_logger) << "connect smtp server: " << host << ":" << port << " ssl=" << ssl << " fail";
        return nullptr;
    }
    std::string buf;
    buf.resize(1024);
...
SmtpResult::ptr SmtpClient::doCmd(const std::string& cmd, bool debug) {
    ...
    if (code >= 400) {
        return std::make_shared<SmtpResult>(code, sylar::replace(buf.substr(buf.find(' ') + 1), "\r\n", ""));
    }
    return nullptr;
}
