...
bool Redis::connect(const std::string& ip, int port, uint64_t ms) {
    ...
    if (c) {
        ...
        if (!m_passwd.empty()) {
            ...
            if (!r) {
                SYLAR_LOG_ERROR(g_logger) << "auth error:(" << m_host << ":" << m_port << ", " << m_name << ")";
                return false;
            }
            if (r->type != REDIS_REPLY_STATUS) {
                SYLAR_LOG_ERROR(g_logger)
                    << "auth reply type error:" << r->type << "(" << m_host << ":" << m_port << ", " << m_name << ")";
                return false;
            }
            if (!r->str) {
                SYLAR_LOG_ERROR(g_logger)
                    << "auth reply str error: NULL(" << m_host << ":" << m_port << ", " << m_name << ")";
                return false;
            }
            if (strcmp(r->str, "OK") == 0) {
                ...
                SYLAR_LOG_ERROR(g_logger)
                    << "auth error: " << r->str << "(" << m_host << ":" << m_port << ", " << m_name << ")";
                return false;
            }
        }
        return true;
...
ReplyPtr Redis::cmd(const char* fmt, va_list ap) {
    ...
    if (!r) {
        if (m_logEnable) {
            SYLAR_LOG_ERROR(g_logger) << "redisCommand error: (" << fmt << ")(" << m_host << ":" << m_port << ")("
                                      << m_name << ")";
        }
        return nullptr;
    }
...
void FoxRedis::OnAuthCb(redisAsyncContext* c, void* rp, void* priv) {
    ...
    if (!r) {
        SYLAR_LOG_ERROR(g_logger) << "auth error:(" << fr->m_host << ":" << fr->m_port << ", " << fr->m_name << ")";
        return;
    }
    if (r->type != REDIS_REPLY_STATUS) {
        SYLAR_LOG_ERROR(g_logger) << "auth reply type error:" << r->type << "(" << fr->m_host << ":" << fr->m_port
                                  << ", " << fr->m_name << ")";
        return;
    }
    if (!r->str) {
        SYLAR_LOG_ERROR(g_logger) << "auth reply str error: NULL(" << fr->m_host << ":" << fr->m_port << ", "
                                  << fr->m_name << ")";
        return;
    }
    if (strcmp(r->str, "OK") == 0) {
        ...
        SYLAR_LOG_ERROR(g_logger) << "auth error: " << r->str << "(" << fr->m_host << ":" << fr->m_port << ", "
                                  << fr->m_name << ")";
    }
}
...
void FoxRedis::ConnectCb(const redisAsyncContext* c, int status) {
    ...
    if (!status) {
        SYLAR_LOG_INFO(g_logger) << "FoxRedis::ConnectCb " << c->c.tcp.host << ":" << c->c.tcp.port << " success";
...
void FoxRedis::pcmd(FCtx* fctx) {
    if (m_status == UNCONNECTED) {
        SYLAR_LOG_INFO(g_logger) << "redis (" << m_host << ":" << m_port << ") unconnected " << fctx->cmd;
        init();
        if (fctx->fiber) {
...
void FoxRedis::Ctx::EventCb(int fd, short event, void* d) {
    ...
    if (ctx->rds->m_logEnable) {
        ...
        SYLAR_LOG_INFO(g_logger) << "redis cmd: '" << ctx->cmd << "' reach timeout "
                                 << (ctx->rds->m_cmdTimeout.tv_sec * 1000 + ctx->rds->m_cmdTimeout.tv_usec / 1000)
                                 << "ms";
    }
    if (ctx->fctx->fiber) {
        ctx->fctx->scheduler->schedule(&ctx->fctx->fiber);
...
void FoxRedisCluster::OnAuthCb(redisClusterAsyncContext* c, void* rp, void* priv) {
    ...
    if (r->type != REDIS_REPLY_STATUS) {
        SYLAR_LOG_ERROR(g_logger) << "auth reply type error:" << r->type << "(" << fr->m_host << ", " << fr->m_name
                                  << ")";
        return;
    }
    if (!r->str) {
        SYLAR_LOG_ERROR(g_logger) << "auth reply str error: NULL(" << fr->m_host << ", " << fr->m_name << ")";
        return;
    }
    if (strcmp(r->str, "OK") == 0) {
        SYLAR_LOG_INFO(g_logger) << "auth ok: " << r->str << "(" << fr->m_host << ", " << fr->m_name << ")";
    } else {
        SYLAR_LOG_ERROR(g_logger) << "auth error: " << r->str << "(" << fr->m_host << ", " << fr->m_name << ")";
    }
}
...
void FoxRedisCluster::ConnectCb(const redisAsyncContext* c, int status) {
    ...
    if (!status) {
        SYLAR_LOG_INFO(g_logger) << "FoxRedisCluster::ConnectCb " << c->c.tcp.host << ":" << c->c.tcp.port
                                 << " success";
        if (!ar->m_passwd.empty()) {
            int rt = redisClusterAsyncCommand(ar->m_context.get(), FoxRedisCluster::OnAuthCb, ar, "auth %s",
                                              ar->m_passwd.c_str());
            if (rt) {
        ...
        SYLAR_LOG_ERROR(g_logger) << "FoxRedisCluster::ConnectCb " << c->c.tcp.host << ":" << c->c.tcp.port
                                  << " fail, error:" << c->errstr;
    }
}

void FoxRedisCluster::DisconnectCb(const redisAsyncContext* c, int status) {
    SYLAR_LOG_INFO(g_logger) << "FoxRedisCluster::DisconnectCb " << c->c.tcp.host << ":" << c->c.tcp.port
                             << " status:" << status;
}
...
bool FoxRedisCluster::pinit() {
    ...
    if (!ctx) {
        SYLAR_LOG_ERROR(g_logger) << "redisClusterAsyncConnect (" << m_host << ") null";
        return false;
    }
    if (ctx->err) {
        SYLAR_LOG_ERROR(g_logger) << "Error:(" << ctx->err << ")" << ctx->errstr << " passwd=" << m_passwd;
        return false;
    }
    m_status = CONNECTED;
    // m_context.reset(ctx, redisAsyncFree);
