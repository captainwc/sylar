...
bool ZKServiceDiscovery::existsOrCreate(const std::string& path) {
    ...
    if (v == ZOK) {
        ...
        if (pos == 0 || existsOrCreate(path.substr(0, pos))) {
            ...
            if (v != ZOK) {
                SYLAR_LOG_ERROR(g_logger) << "create path=" << path << " error:" << zerror(v) << " (" << v << ")";
                return false;
            }
            return true;
        }
...
bool ZKServiceDiscovery::registerInfo(const std::string& domain, const std::string& service,
    ...
    if (!m_isOnTimer) {
        SYLAR_LOG_ERROR(g_logger) << "create path=" << (path + "/" + ip_and_port) << " fail, error:" << zerror(rt)
                                  << " (" << rt << ")";
    }
...
bool ZKServiceDiscovery::queryInfo(const std::string& domain, const std::string& service) {
    if (service != "all") {
        ...
        if (!m_isOnTimer) {
            SYLAR_LOG_ERROR(g_logger) << "create path=" << (path + "/" + m_selfInfo) << " fail, error:" << zerror(rt)
                                      << " (" << rt << ")";
        }
        return rt == ZNODEEXISTS;
    } else {
...
bool ZKServiceDiscovery::getChildren(const std::string& path) {
    ...
    }
    {
        ...
        if (it == m_queryInfos.end()) {
            SYLAR_LOG_ERROR(g_logger) << "get_children path=" << path << " domian=" << domain << " not exists";
            return false;
        }
        if (it->second.count(service) == 0 && it->second.count("all") == 0) {
            SYLAR_LOG_ERROR(g_logger) << "get_children path=" << path << " service=" << service << " not exists "
    ...
    if (v != ZOK) {
        SYLAR_LOG_ERROR(g_logger) << "get_children path=" << path << " fail, error:" << zerror(v) << " (" << v << ")";
        return false;
    }
    ...
    for (auto& i : vals) {
        ...
        SYLAR_LOG_INFO(g_logger) << "domain=" << domain << " service=" << service << " info=" << info->toString();
    }
...
void ZKServiceDiscovery::onWatch(int type, int stat, const std::string& path, ZKClient::ptr client) {
    ...
    SYLAR_LOG_ERROR(g_logger) << "onWatch hosts=" << m_hosts << " type=" << type << " stat=" << stat << " path=" << path
                              << " client=" << client;
}
