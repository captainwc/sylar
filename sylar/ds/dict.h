...
namespace ds {
    ...
    public:
        ...
        std::ostream& dump(std::ostream& os) {
            ...
            os << "[Dict total=" << m_total << " bucket=" << m_size << " rate=" << getRate() << "]" << std::endl;
            return os;
        }
    ...
    public:
        ...
        SharedArray<char> getRaw(const uint64_t& id, bool duplicate = true) { return m_dict.get(id, duplicate); }
        ...
        void foreach (std::function<bool(const uint64_t& k, const char* v, size_t size)> cb) { m_dict.foreach (cb); }
