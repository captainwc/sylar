...
namespace orm {
        ...
        do {
            ...
            for (auto& x : cnames) {
                if (col_names.count(x) == 0) {
                    SYLAR_LOG_ERROR(g_logger)
                        << "table name=" << m_name << " idx=" << idx_ptr->getName() << " col=" << x << " not exists";
                    return false;
                }
            }
    ...
    void Table::gen_dao_inc(std::ofstream& ofs) {
        ...
        ofs << "    static int Insert(" << GetAsClassName(class_name) << "::ptr info, " << m_updateclass
            << "::ptr conn);" << std::endl;
        ofs << "    static int InsertOrUpdate(" << GetAsClassName(class_name) << "::ptr info, " << m_updateclass
            << "::ptr conn);" << std::endl;
    ...
    void Table::gen_dao_src(std::ofstream& ofs) {
        std::string class_name     = m_name + m_subfix;
        std::string class_name_dao = class_name + "_dao";
        ofs << "int " << GetAsClassName(class_name_dao) << "::Update(" << GetAsClassName(class_name) << "::ptr info, "
...
#define CHECK_STMT(v)                                                           \
    ...
    ofs << "    }" << std::endl;
        ...
        ofs << "int " << GetAsClassName(class_name_dao) << "::Insert(" << GetAsClassName(class_name) << "::ptr info, "
            << m_updateclass << "::ptr conn) {" << std::endl;
        ...
        ofs << "int " << GetAsClassName(class_name_dao) << "::Delete(" << GetAsClassName(class_name) << "::ptr info, "
            << m_updateclass << "::ptr conn) {" << std::endl;
...
#define PARSE_OBJECT(prefix)                                                           \
    for (size_t i = 0; i < m_cols.size(); ++i) {                                       \
        ...
        ofs << "int " << GetAsClassName(class_name_dao) << "::CreateTableSQLite3(" << m_dbclass << "::ptr conn) {"
            << std::endl;
        ...
        ofs << "int " << GetAsClassName(class_name_dao) << "::CreateTableMySQL(" << m_dbclass << "::ptr conn) {"
            << std::endl;
        ofs << "    return conn->execute(\"CREATE TABLE " << m_name << "(\"" << std::endl;
        is_first = true;
        for (auto& i : m_cols) {
