...
RockServer::RockServer(const std::string& type, sylar::IOManager* worker, sylar::IOManager* io_worker,
...
void RockServer::handleClient(Socket::ptr client) {
    ...
    session->setRequestHandler(
        [](sylar::RockRequest::ptr req, sylar::RockResponse::ptr rsp, sylar::RockStream::ptr conn) -> bool {
    ...
    session->setNotifyHandler([](sylar::RockNotify::ptr nty, sylar::RockStream::ptr conn) -> bool {
        SYLAR_LOG_INFO(g_logger) << "handleNty " << nty->toString() << " body=" << nty->getBody();
        bool rt = false;
