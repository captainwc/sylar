...
void on_watcher(int type, int stat, const std::string& path, sylar::ZKClient::ptr client) {
    SYLAR_LOG_INFO(g_logger) << " type=" << type << " stat=" << stat << " path=" << path << " client=" << client
                             << " fiber=" << sylar::Fiber::GetThis() << " iomanager=" << sylar::IOManager::GetThis();
...
int main(int argc, char** argv) {
    ...
    if (g_argc > 1) {
        ...
        // SYLAR_LOG_INFO(g_logger) << client->init("127.0.0.1:21811,127.0.0.1:21812,127.0.0.1:21811", 3000,
        // on_watcher);
