...
void test_one() {
    ...
    int32_t len2 =
        sylar::CryptoUtil::AES128Cbc(key.c_str(), iv.c_str(), encode.c_str(), encode.size(), &decode[0], false);
    if (len2 >= 0) {
        decode.resize(len2);
    }
...
void test_rsa() {
    ...
    std::cout << "rsa pubsize: " << rsa->getPubRSASize() << " prisize: " << rsa->getPriRSASize() << std::endl;
